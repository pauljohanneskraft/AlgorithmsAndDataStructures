...
public struct DoublyLinkedList < Element > : List {
	...
	public subscript(index: Int) -> Element? {
		...
		set {
			...
			current!.next = DoublyLinkedItem(data: newValue!, prev: current, next: next?.next)
		}
	}
	...
	public mutating func insert(_ data: Element, at index: Int) throws {
		...
		current!.next = DoublyLinkedItem(data: data, prev: current, next: next)
	}
	...
	private var root : DoublyLinkedItem < Element >?
...
private class DoublyLinkedItem<Element> : CustomStringConvertible {
	let data : Element
